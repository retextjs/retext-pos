...
retext = new Retext();
retextWithPOS = new Retext().use(pos);
...
suite('retext w/o retext-pos', function () {
...
suite('retext w/ retext-pos', function () {
    bench('A paragraph (5 sentences, 100 words)', function (done) {
        retextWithPOS.parse(paragraph, done);
    });

    bench('A section (10 paragraphs, 50 sentences, 1,000 words)',
        function (done) {
            retextWithPOS.parse(section, done);
        }
    );
});
